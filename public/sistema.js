...
const btnEliminarHechas = document.querySelector(
  "[value='Eliminar tareas hechas']"
);
...
function marcarHecha() {
  ...
  for (let i = 0; i < tareasPendientes.length; i++) {
    ...
    const checkbox = tareasPendientes[i].querySelector(
      "input[type='checkbox']"
    );
    if (checkbox.checked) {
...
// Función para eliminar las tareas hechas
